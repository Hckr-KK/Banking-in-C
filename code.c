...
int main()
{
    ...
    printf("______Welcome To Bank of Baroda Online Payment System______\n\n");
...
void accept(struct customer list[80], int s)
{
    ...
    for (i = 0; i < s; i++)
    {
        printf("\nEnter data for Record #%d", i + 1);
            ...
            kk:
            printf("\nEnter account_no : ");
            scanf("%llu", &list[i].account_no);
            
            int temp = 0;
            unsigned long long x = list[i].account_no; 
            

            while(list[i].account_no != 0)
            {

                    list[i].account_no = list[i].account_no/10;
                    temp = temp + 1;
            }
        ...
        if(temp == 12)
        {
            ...
            printf("\nPlease entr a valid 12 digits account number\n");
            goto kk;
        }
        fuck2:
        fflush(stdin);
...
void withdraw(struct customer list[], int s, int number, int amt)
{
    ...
    printf("Platform Created By KK\t");
    printf("Thank You For Using BOB Online Platfrom\n\n");
}
